...
debug_task(void)
{
	...
	for (;;) {
		...
		double chassis_left_speed =
			(fabs(front_left->getActualVelocity()) +
			 fabs(back_left->getActualVelocity())) /
			2;
		double chassis_right_speed =
			(fabs(front_right->getActualVelocity()) +
			 fabs(back_right->getActualVelocity())) /
			2;

		double chassis_left_pos =
			(front_left->getPosition() + back_left->getPosition()) /
			2;
		double chassis_right_pos = (front_right->getPosition() +
					    back_right->getPosition()) /
					   2;

		lv_gauge_set_value(chassis_left_speed_gauge,
				   0,
				   fabs(chassis_left_speed));
		lv_gauge_set_value(chassis_right_speed_gauge,
				   0,
				   fabs(chassis_right_speed));
		...
		lv_label_set_text(chassis_left_label,
				  chassis_left_speed_str.c_str());
		lv_label_set_text(chassis_right_label,
				  chassis_right_speed_str.c_str());

		/*            CHASSIS GRAPH TAB            */

		lv_chart_set_next(chassis_chart, chassis_max_speed, 200);
		lv_chart_set_next(chassis_chart,
				  chassis_left_current_speed,
				  fabs(chassis_left_speed));
		lv_chart_set_next(chassis_chart,
				  chassis_right_current_speed,
				  fabs(chassis_right_speed));

		/*            INTAKE TAB            */

		double intake_front_speed =
			(fabs(left_intake->getActualVelocity()) +
			 fabs(right_intake->getActualVelocity())) /
			2;
		double intake_internal_speed =
			(fabs(top_intake->getActualVelocity()) +
			 fabs(bottom_intake->getActualVelocity())) /
			2;

		lv_gauge_set_value(intake_front_gauge,
				   0,
				   fabs(intake_front_speed));
		lv_gauge_set_value(intake_internal_gauge,
				   0,
				   fabs(intake_internal_speed));

		double intake_front_pos = (left_intake->getPosition() +
					   right_intake->getPosition()) /
					  2;
		double intake_internal_pos = (top_intake->getPosition() +
					      bottom_intake->getPosition()) /
					     2;
		...
		lv_label_set_text(intake_internal_label,
				  intake_internal_str.c_str());

		/*            INTAKE GRAPH TAB            */

		lv_chart_set_next(intake_chart, intake_max_speed, 200);
		lv_chart_set_next(intake_chart,
				  intake_front_current_speed,
				  intake_front_speed);
		lv_chart_set_next(intake_chart,
				  intake_internal_current_speed,
				  intake_internal_speed);
		...
		std::string pos =
			"global_x: " +
			std::to_string(
				drive->getState().x.convert(okapi::inch)) +
			"_in\n" + "global_y: " +
			std::to_string(
				drive->getState().y.convert(okapi::inch)) +
			"_in\n" + "global_theta: " +
			std::to_string(drive->getState().theta.convert(
				okapi::degree)) +
			"_deg\n";
		...
		std::string chassis_health_str =
			"Chassis\n\nfront left: " +
			checkhealth::motor(front_left) + "\n" +
			"front right: " + checkhealth::motor(front_right) +
			"\n" + "back left: " + checkhealth::motor(back_left) +
			"\n" + "back right: " + checkhealth::motor(back_right);

		lv_label_set_text(chassis_health_label,
				  chassis_health_str.c_str());

		std::string intake_health_str =
		...
		lv_label_set_text(intake_health_label,
				  intake_health_str.c_str());

		std::string core_health_str =
			"Core\n\ncontroller: " +
			checkhealth::controller(master) + "\n" +
	   ...
	   pros::controller_digital_e_t right)
{
	...
	if (master.get_digital(left) || master.get_digital(right)) {
		...
		case CHASSIS_GRAPH_TAB:
			lv_tabview_set_tab_act(tabview,
					       CHASSIS_GRAPH_TAB,
					       false);
			break;
		case INTAKE_TAB:
...
init(void)
{
	...
	lv_obj_align(chassis_left_speed_gauge,
		     NULL,
	...
	lv_obj_align(chassis_right_speed_gauge,
		     NULL,
	...
	chassis_left_current_speed =
		lv_chart_add_series(chassis_chart, LV_COLOR_LIME);
	chassis_right_current_speed =
		lv_chart_add_series(chassis_chart, LV_COLOR_CYAN);
	...
	intake_front_current_speed =
		lv_chart_add_series(intake_chart, LV_COLOR_LIME);
	intake_internal_current_speed =
		lv_chart_add_series(intake_chart, LV_COLOR_CYAN);
	...
	if (!debugTask) {
		debugTask = std::make_shared<pros::Task>(debug_task,
							 "debug task (xenon)");
